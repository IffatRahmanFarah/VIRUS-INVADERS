...
enum Weather { SUNNY,NIGHT, WINTER, RAIN, NIGHT_WITH_RAIN };  // Added
...
// Array to hold fireflies
Firefly fireflies[4]; // 3-4 fireflies
int numFireflies = 4;

void spawnFireflies() {
    for (int i = 0; i < numFireflies; i++) {
        fireflies[i].pos.x = randRange(-0.9f, 0.9f);
        fireflies[i].pos.y = randRange(-0.9f, 0.9f);
    }
}

void updateFireflies() {
    for (int i = 0; i < numFireflies; i++) {
        fireflies[i].pos.x += randRange(-fireflies[i].speed, fireflies[i].speed);
        fireflies[i].pos.y += randRange(-fireflies[i].speed, fireflies[i].speed);

        // Keep fireflies within screen bounds
        fireflies[i].pos.x = clampf(fireflies[i].pos.x, -0.9f, 0.9f);
        fireflies[i].pos.y = clampf(fireflies[i].pos.y, -0.9f, 0.9f);
    }
}

void drawFireflies() {
    glColor3f(1.0f, 1.0f, 0.0f); // Yellow for fireflies
    for (int i = 0; i < numFireflies; i++) {
        drawCircle(fireflies[i].pos.x, fireflies[i].pos.y, fireflies[i].size);
    }
}
...
void drawSnowOnBushes() {
    for (int i = 0; i < 10; i++) {  // Assuming you have 10 bushes
        // Randomly add snow on top of each bush
        float snowX = bushes[i].pos.x + randRange(-0.02f, 0.02f);
        float snowY = bushes[i].pos.y + bushes[i].size + 0.05f; // Slightly above the bush
        float snowSize = randRange(0.01f, 0.03f); // Random snow size

        glColor3f(1.0f, 1.0f, 1.0f); // White snow color
        drawCircle(snowX, snowY, snowSize, 8); // Draw the snow circle on top of the bush
    }}

void drawWeather() {
    // Based on level, show the corresponding weather
    if (currentWeather == SUNNY) {
        glColor3f(0.08f, 0.45f, 0.08f); // Green for sunny park
        ...
        // Add fireflies
        drawFireflies();  // Fireflies in night
    }
    else if (currentWeather == WINTER) {
        glClearColor(0.8f, 0.8f, 1.0f, 1.0f); // Light blue for winter
        drawText(-0.8f, 0.8f, "Weather: Winter");
        drawSnowflakes(); // Snowflakes in winter
        drawSnowOnBushes(); // Snow on bushes in winter
    }
    else if (currentWeather == RAIN) {
        glColor3f(0.6f, 0.4f, 0.2f); // Muddy brown for rainy season
        drawText(-0.8f, 0.8f, "Weather: Rain");
        drawRaindrops(); // Raindrops during rain
    }
    else if (currentWeather == NIGHT_WITH_RAIN) {
        glColor3f(0.1f, 0.1f, 0.3f); // Dark blue for night with rain
        drawText(-0.8f, 0.8f, "Weather: Night with Rain");
        drawRaindrops();  // Draw raindrops for rain
        drawFireflies();  // Draw fireflies for night
    }
}
...
void drawParkBackground() {
    // Drawing the gray-colored pathway (This stays constant across all levels)
    glColor3f(0.5f, 0.5f, 0.5f); // Gray color for the pathway
    glBegin(GL_QUADS);
    ...
    glVertex2f(-pathWidth / 2, -1.0f);
    glEnd();
    drawFireflies(); // Draw fireflies if it's night

    // Background based on the level/weather
    if (currentWeather == SUNNY) {
        // Sunny: Green park background
        glBegin(GL_QUADS);
        glColor3f(0.08f, 0.45f, 0.08f); // Green for sunny park
        glVertex2f(-1.0f, -1.0f);
        glColor3f(0.20f, 0.72f, 0.20f); // Lighter green for sunny park
        glVertex2f(1.0f, -1.0f);
        glVertex2f(1.0f, 1.0f);
        glColor3f(0.08f, 0.45f, 0.08f); // Darker green for sunny park
        glVertex2f(-1.0f, 1.0f);
        glEnd();
    }
    else if (currentWeather == NIGHT) {
        ...
        glColor3f(0.1f, 0.1f, 0.3f); // Dark blue for night
        glVertex2f(-1.0f, -1.0f);
        glColor3f(0.2f, 0.2f, 0.5f); // Lighter blue for night sky
        glVertex2f(1.0f, -1.0f);
        glVertex2f(1.0f, 1.0f);
        glColor3f(0.1f, 0.1f, 0.3f); // Darker blue for night ground
        glVertex2f(-1.0f, 1.0f);
        glEnd();
        // Add fireflies for night
        drawFireflies();
    }
    else if (currentWeather == WINTER) {
        // Winter: Blue background with snowflakes and snow on bushes
        glClearColor(0.8f, 0.8f, 1.0f, 1.0f); // Light blue for winter
        glBegin(GL_QUADS);
        glColor3f(0.5f, 0.5f, 0.7f); // Bluish for winter background
        glVertex2f(-1.0f, -1.0f);
        glColor3f(0.8f, 0.8f, 1.0f); // Light icy blue
        glVertex2f(1.0f, -1.0f);
        glVertex2f(1.0f, 1.0f);
        glColor3f(0.5f, 0.5f, 0.7f); // Bluish for snow
        glVertex2f(-1.0f, 1.0f);
        glEnd();
        // Add snowflakes for winter
        drawSnowflakes();
        // Add snow on bushes
        drawSnowOnBushes();
    }
    else if (currentWeather == RAIN) {
        // Rainy: Muddy brown background
        glBegin(GL_QUADS);
        glColor3f(0.6f, 0.4f, 0.2f); // Muddy brown background
        glVertex2f(-1.0f, -1.0f);
        glColor3f(0.8f, 0.6f, 0.4f); // Lighter brown for rainy ground
        glVertex2f(1.0f, -1.0f);
        glVertex2f(1.0f, 1.0f);
        glColor3f(0.6f, 0.4f, 0.2f); // Darker brown for rainy sky
        glVertex2f(-1.0f, 1.0f);
        glEnd();
        // Draw raindrops for rain
        drawRaindrops();
    }
    else if (currentWeather == NIGHT_WITH_RAIN) {
        // Night with Rain: Dark blue night with rain and fireflies
        glBegin(GL_QUADS);
        glColor3f(0.1f, 0.1f, 0.3f); // Dark blue for night
        glVertex2f(-1.0f, -1.0f);
        glColor3f(0.2f, 0.2f, 0.5f); // Lighter blue for night sky
        ...
        // Draw raindrops for rain
        drawRaindrops();
        // Add fireflies for night with rain
        drawFireflies();
        // Add rain and night effects for level 5
        drawText(-0.8f, 0.8f, "Weather: Night with Rain");
    }
...
// Drawing the paths
    ...
    glVertex2f(-1.0f, -pathWidth/2);
    glEnd();
    // In the path drawing section of drawParkBackground():
...
void drawCartoonHuman(Vec2 pos, float scale = 0.1f) {
    ...
    // --- Spray bottle ---
// --- Spray bottle (scaled 0.5) ---
glColor3f(0.2f, 0.6f, 1.0f); // light blue

float scales = 1.2f;

float Ax = pos.x - 0.055f * scales;
float Ay = pos.y - 0.002f * scales;
float Bx = pos.x - 0.051f * scales;
float By = pos.y - 0.004f * scales;

// calculate radius
float radius = sqrt((Bx - Ax)*(Bx - Ax) + (By - Ay)*(By - Ay));
drawCircle(Ax, Ay, radius, 100); // smoother circle

glColor3f(1.0f, 0.5f, 0.0f); // orange for polygon
glBegin(GL_POLYGON);
glVertex2f(pos.x - 0.0551523822397f * scales, pos.y - 0.0024280575178f * scales); // A
glVertex2f(pos.x - 0.0595958001984f * scales, pos.y - 0.0024427642443f * scales); // C
glVertex2f(pos.x - 0.0595527826559f * scales, pos.y + 0.0180383273451f * scales); // D
glVertex2f(pos.x - 0.0567689387578f * scales, pos.y + 0.0200551938427f * scales); // E
glVertex2f(pos.x - 0.054f * scales,           pos.y + 0.02f * scales);           // F
glVertex2f(pos.x - 0.0508035589763f * scales, pos.y + 0.0180383273451f * scales); // G
glVertex2f(pos.x - 0.0507f * scales,          pos.y - 0.00247f * scales);         // H
glVertex2f(pos.x - 0.0551523822397f * scales, pos.y - 0.0024280575178f * scales); // A
glEnd();
...
void spawnSprayBurst() {
    float scales = 1.1f; // same scale as bottle

    // calculate bottle tip position
    float Ax = player.pos.x - 0.055f * scales;
    float Ay = player.pos.y - 0.002f * scales;
    float Bx = player.pos.x - 0.051f * scales;
    float By = player.pos.y - 0.004f * scales;
    float radius = sqrt((Bx - Ax)*(Bx - Ax) + (By - Ay)*(By - Ay));

    float tipX = Ax - radius; // x-coordinate of nozzle tip
    float tipY = Ay;           // y-coordinate of nozzle tip

    for (int i = 0; i < 8; i++) {
        Spray s;
        s.pos = { tipX + randRange(-0.005f * scales, 0.005f * scales),
                  tipY + randRange(-0.005f * scales, 0.005f * scales) };

        float angle = PI + randRange(-PI / 8.0f, PI / 8.0f);
        float speed = randRange(0.015f * scales, 0.03f * scales);
        s.vel = { cosf(angle) * speed, sinf(angle) * speed };

        s.r = 0.01f * scales;  // scale the radius of spray
...
void updateGame() {
    ...
    if (level == 1) {
        currentWeather = SUNNY;   // Level 1: SUNNY
    } else if (level == 2) {
        currentWeather = NIGHT;   // Level 2: NIGHT
    } else if (level == 3) {
        currentWeather = WINTER;  // Level 3: WINTER
    } else if (level == 4) {
        currentWeather = RAIN;    // Level 4: RAIN
    } else if (level == 5) {
        currentWeather = NIGHT_WITH_RAIN;  // Level 5: NIGHT_WITH_RAIN
    }

    // Weather-specific updates (e.g., rain, snow) for levels 1-4
    if (level < 5) {  // Skip rain and snow effects in Level 5
        if (currentWeather == WINTER) {
            updateSnowflakes();  // Only update snowflakes for winter
        }

        if (currentWeather == RAIN) {
            updateRaindrops();   // Only update raindrops for rain
        }
    }

    // Level 5 - Night with Rain: Update raindrops and fireflies
    if (level == 5) {
        updateRaindrops();  // Update raindrops for Night with Rain
        drawRaindrops();    // Draw raindrops for Night with Rain
        drawFireflies();    // Draw fireflies for night
    }

    // Continue with the rest of your game logic
    // Update player, viruses, and other entities
    // Handle spray, collisions, etc.
    // Spawn new viruses, vaccines, etc.
...

{
    ...
    PlaySound("bensound-realworld (online-audio-converter (mp3cut.net).wav", NULL,SND_ASYNC|SND_FILENAME|SND_LOOP);
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("VIRUS INVADERS ");
